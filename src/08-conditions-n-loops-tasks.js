...
function doRectanglesOverlap(rect1, rect2) {
  if ((rect1.top + rect1.height < rect2.top) || (rect1.left + rect1.width < rect2.left)
    || (rect1.left > rect2.left + rect2.width) || (rect1.top > rect2.top + rect2.height)) {
    return false;
  }
  return true;
}
...
function isCreditCardNumber(ccn) {
  let strCard = ccn.toString();
  let sum = 0;
  for (let i = 0; i < strCard.length; i++) {
    let num = +strCard[i];
    if ((strCard.length - i) % 2 === 0) {
      num *= 2;
      if (num > 9) {
        num -= 9;
      }
...
function getCommonDirectoryPath(pathes) {
  ...
  res = pathes[0][0];
  console.log(res);
  while (res.length < pathes[0].length - 1) {
    for (let j = 0; j < pathes.length; j++) {
      for (let i = 0; i < res.length; i++) {
        if (res !== pathes[j].slice(0, res.length) || res.length === 1) {
          return '';
        }
        else {
          return res.slice(0, (res.lastIndexOf('/') === -1) ? res.length : res.lastIndexOf('/') + 1);
        }

      }
      if (j === pathes.length - 1) {
        res += pathes[0][res.length];
      }
    }
  }
  return res.slice(0, (res.lastIndexOf('/') === -1) ? res.length : res.lastIndexOf('/') + 1)
